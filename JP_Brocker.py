...
def generar_grafico_radar():
    ...
    fig, ax = plt.subplots(figsize=(4, 4), subplot_kw=dict(polar=True), facecolor='#0A192F')
    ax.set_facecolor('#0A192F')
    
    ax.plot(angles, stats, color='#D4AF37', linewidth=2, linestyle='solid')
    ax.fill(angles, stats, color='#D4AF37', alpha=0.3)
    
    ax.set_yticklabels([])
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(labels, size=8, color='#FFFFFF', fontweight='bold')
    
    ax.spines['polar'].set_color('#D4AF37')
    ax.grid(color='#4A5568', linestyle='--', linewidth=0.7)
    
    temp_file = tempfile.NamedTemporaryFile(delete=False, suffix='.png')
    plt.tight_layout()
    plt.savefig(temp_file.name, format='png', dpi=200, bbox_inches='tight', facecolor=fig.get_facecolor(), edgecolor='none')
    plt.close(fig)
    return temp_file.name

def generar_pdf_consultoria(fila_client):
    ...
    empresa = buscar_col(["empresa", "negocio", "organización"], "Escala Consulting")
    ...
    pdf.cell(0, 8, "Informe Ejecutivo de Evaluación de Cuenta", 0, 1, "L")
    
    pdf.set_font("helvetica", "B", 9)
    pdf.set_text_color(100, 100, 100)
    pdf.cell(0, 5, f"CLIENTE / REPRESENTANTE: {{representante}}", 0, 1, "L")
    pdf.cell(0, 5, f"RAZÓN SOCIAL / COMERCIAL: {{empresa}}", 0, 1, "L")
    pdf.cell(0, 5, f"FECHA DE EMISIÓN: {{datetime.now().strftime('%d de %B, %Y')}}", 0, 1, "L")
    pdf.ln(4)
    ...
    pdf.multi_cell(0, 4.5, "La empresa presenta una condición de Vulnerabilidad Estructural Crítica (Índice de Salud de Gestión: 28/100). El diagnóstico revela una alta dependencia operativa del fundador y una tensión severa en la liquidez a corto plazo, amenazando la sostenibilidad del negocio y obstaculizando la meta estratégica.")
    pdf.ln(4)
    
    pdf.set_font("helvetica", "B", 10)
    pdf.set_text_color(10, 37, 64)
    pdf.cell(0, 6, "1. Índice de Madurez de Gestión (Maturity Assessment)", 0, 1, "L")
    ...
    pdf.cell(35, 5.5, "Pilar Estratégico", 1, 0, "C", True)
    pdf.cell(75, 5.5, "Variable Evaluada", 1, 0, "C", True)
    pdf.cell(25, 5.5, "Puntuación", 1, 0, "C", True)
    pdf.cell(45, 5.5, "Estado Técnico", 1, 1, "C", True)

    pdf.set_font("helvetica", "", 8)
    pdf.set_text_color(0, 0, 0)
    
    datos_tabla = [
        ("Comercial", "Embudo de ventas & Definición de Avatar", "35%", "Incipiente / Intuitivo"),
        ("Financiero", "Flujo de caja, P&L & Separación patrimonial", "20%", "Crítico / Hemorragia"),
        ("Operativo", "Dependencia del fundador & Procesos", "15%", "Colapso por Autoempleo"),
        ("Legal & Gobierno", "Tributación, Contratos & Activos", "40%", "Riesgo Moderado")
    ]
   ...
   # ---------------------------------------------------------
    ...
    pdf.set_text_color(10, 37, 64)
    pdf.cell(0, 6, "2. Análisis Técnico por Pilares (Diagnóstico Profundo)", 0, 1, "L")
    pdf.ln(2)
    ...
    pdf.set_font("helvetica", "", 8.5)
    pdf.set_text_color(50, 50, 50)
    pdf.multi_cell(0, 4, "El modelo de adquisición descansa enteramente en la intuición comercial y en esfuerzos de redes sociales sin un embudo (funnel) estructurado. La ausencia de un perfil de cliente ideal (avatar) documentado genera altos costos de adquisición de clientes (CAC).")
    pdf.ln(3)
    ...
    pdf.set_font("helvetica", "", 8.5)
    pdf.set_text_color(50, 50, 50)
    pdf.multi_cell(0, 4, "El negocio exhibe el nivel máximo de dependencia operativa del fundador (5/5). La organización opera bajo un esquema de 'cultura de memoria', donde ningún proceso clave cuenta con manuales o procedimientos documentados.")
    pdf.ln(3)
    ...
    pdf.set_font("helvetica", "", 8.5)
    pdf.set_text_color(50, 50, 50)
    pdf.multi_cell(0, 4, "Se identifican pendientes en obligaciones tributarias y una práctica generalizada de operar mediante acuerdos de palabra con proveedores y clientes en lugar de contratos escritos y blindados.")
    ...
    pdf.set_font("helvetica", "B", 11)
    pdf.set_text_color(10, 37, 64)
    pdf.cell(0, 6, "3. Hoja de Ruta Estratégica (Plan de Trabajo 90 Días)", 0, 1, "L")
    pdf.ln(2)
    
    pdf.set_font("helvetica", "B", 9)
    pdf.set_text_color(212, 175, 55)
    pdf.cell(0, 5, "Fase 1: Estabilización de Urgencias y Caja (Mes 1)", 0, 1, "L")
    pdf.set_font("helvetica", "", 8.5)
    pdf.set_text_color(50, 50, 50)
    pdf.multi_cell(0, 4, "Objetivo: Detener la quema de efectivo y blindar el patrimonio.\n- Establecer cuentas bancarias corporativas 100% separadas de las personales.\n- Implementar un Flujo de Caja Operativo diario a 13 semanas.\n- Auditoría tributaria exprés para sanear pendientes fiscales.")
    pdf.ln(3)
    
    pdf.set_font("helvetica", "B", 9)
    pdf.set_text_color(212, 175, 55)
    pdf.cell(0, 5, "Fase 2: Reingeniería Comercial y Claridad de Oferta (Mes 2)", 0, 1, "L")
    pdf.set_font("helvetica", "", 8.5)
    pdf.set_text_color(50, 50, 50)
    pdf.multi_cell(0, 4, "Objetivo: Estructurar un motor de ventas predecible.\n- Definición estricta del Avatar y propuesta de valor basada en márgenes.\n- Diseño e implementación de un embudo (funnel) comercial medible.\n- Estandarización de contratos base.")
    pdf.ln(3)
    
    pdf.set_font("helvetica", "B", 9)
    pdf.set_text_color(212, 175, 55)
    pdf.cell(0, 5, "Fase 3: Estandarización y Desacoplamiento Operativo (Mes 3)", 0, 1, "L")
    pdf.set_font("helvetica", "", 8.5)
    pdf.set_text_color(50, 50, 50)
    pdf.multi_cell(0, 4, "Objetivo: Reducir la dependencia del fundador y escalar con autonomía.\n- Documentación de los 3 procesos core de entrega de servicio.\n- Automatización de flujos con herramientas en la nube.\n- Establecimiento de un cuadro de mando integral (KPIs).")
...
st.markdown("""
    <style>
    .stApp {
        background: linear-gradient(135deg, #0A192F 0%, #0A2540 100%);
        color: #FFFFFF;
    }
    h1, h2, h3, h4 {
...
# ==========================================
st.markdown("<h1 style='text-align: center; font-size: 2.8rem; margin-bottom: 0; color: #D4AF37 !important;'>🏛️ ESCALA FINANCE</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #FFFFFF; font-size: 1.4rem; font-weight: bold; margin-top: 0;'>Tu consultor financiero de confianza</p>", unsafe_allow_html=True)
st.write("")
...
<div class="card-corporativa">
    <h3 style='margin-top:0; color: #D4AF37 !important;'>✨ Asesoría Patrimonial y Estrategia de Financiamiento</h3>
    <p style='color: #E2E8F0; font-size: 1.1rem;'>Como Bróker especialista, conectamos tus metas con las mejores alternativas del ecosistema de manera independiente, mediante un análisis técnico riguroso y relaciones directas con proveedores institucionales.</p>
    <strong style='color: #D4AF37;'>💼 Nuestra consultoría inicial no genera honorarios para ti</strong> (estos son cubiertos de manera directa por las firmas aliadas del mercado comercial).
</div>
""", unsafe_allow_html=True)
...
with col_izq:
    ...
    with st.form(key="formulario_leads", clear_on_submit=True):
        ...
        st.markdown("""
        <p style='font-size: 0.82rem; color: #CBD5E1; text-align: justify; line-height: 1.25;'>
...
with col_der:
    ...
    <a href="{{url_flujo_completo}}" target="_blank" style="text-decoration: none; color: inherit;">
        <div class="ejecutivo-box">
            ...
            <h4 style="margin: 0; color: #D4AF37; font-size: 1.25rem;">Ec. Jonathan Vaca Cruz</h4>
            <p style="margin: 3px 0 10px 0; color: #10B981; font-weight: bold; font-size: 0.9rem;">💼 Broker & Consultor Financiero Senior</p>
            <div style="background-color: #0A2540; padding: 12px; border-radius: 8px; font-size: 0.88rem; color: #E2E8F0; text-align: justify; border-left: 3px solid #D4AF37;">
...
# ==========================================
# 8. SECCIÓN ADMINISTRATIVA / PANEL DE LEADS Y PDF
# ==========================================
st.markdown("### 🔒 Panel de Gestión y Generación de Informes de Consultoría")
password_ingresado = st.text_input("Contraseña de Acceso al Panel Administrativo:", type="password")

if password_ingresado == PASSWORD_DASHBOARD:
    st.success("✅ Acceso concedido al panel ejecutivo.")
    
    df_leads = leer_leads()
    
    if not df_leads.empty:
        st.markdown("#### 📁 Expedientes y Trámites Ingresados")
        st.dataframe(df_leads, use_container_width=True)
        
        st.markdown("#### 📑 Generación de Informe Técnico PDF de Diagnóstico")
        
        nombres_clientes = df_leads["nombre"].tolist() if "nombre" in df_leads.columns else []
        if nombres_clientes:
            cliente_seleccionado = st.selectbox("Seleccione cliente para emitir reporte:", options=nombres_clientes)
            fila_seleccionada = df_leads[df_leads["nombre"] == cliente_seleccionado].iloc[0]
            
            if st.button("📄 Generar Informe Técnico en PDF"):
                pdf_buffer = generar_pdf_consultoria(fila_seleccionada)
                st.download_button(
                    label="📥 Descargar Informe Ejecutivo PDF",
                    data=pdf_buffer,
                    file_name=f"Informe_Consultoria_{{cliente_seleccionado.replace(' ', '_')}}.pdf",
                    mime="application/pdf"
                )
    else:
        st.info("No hay trámites registrados en la base de datos local todavía.")
elif password_ingresado:
    st.error("❌ Contraseña incorrecta. Por favor verifique sus credenciales.")
...
with st.expander("🔒 Acceso a Panel de Administración y Informes"):
    ...
    # ==========================================
# 11. ENLACE A HERRAMIENTA DE FINANZAS
# ==========================================
st.markdown("---")
st.markdown("### 💼 Herramientas de Gestión")

# Botón estilizado que redirige a la aplicación externa
st.link_button(
    label="🚀 Abrir Herramienta de Finanzas Personales y Negocios",
    url="https://golden-ledger-ai-93.lovable.app/",
